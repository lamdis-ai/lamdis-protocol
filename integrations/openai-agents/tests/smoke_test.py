"""Calls the live sandbox through the tools exactly as the Agents runner would.
No account, no key, no OPENAI_API_KEY, no money.
...
from lamdis_openai_agents import lamdis_check_feasible, lamdis_job_status, lamdis_run_job, lamdis_tools
...
def call(tool, **args) -> dict:
    raw = json.dumps(args)
    ctx = ToolContext(context=None, tool_name=tool.name, tool_call_id="smoke", tool_arguments=raw)
...
def test_tools_are_function_tools():
    ...
    props = lamdis_run_job.params_json_schema["properties"]
    assert props["sandbox"].get("default") is True, props["sandbox"]
    assert "cents" in props["fee_minor"]["description"]
