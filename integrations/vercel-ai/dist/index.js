...
function resolveBase(opts) {
    const env = globalThis.process?.env?.LAMDIS_BASE_URL;
    return (opts.baseUrl ?? env ?? DEFAULT_BASE_URL).replace(/\/+$/, "");
}
async function request(opts, method, path, body, token) {
    ...
    try {
        ...
        return { error: `could not reach the exchange: ${e.message}`, http_status: 0 };
    }
...
export function postTask(input, opts = {}) {
    const body = {
        kind: input.kind ?? "observe", predicate: input.predicate, lat: input.lat, lon: input.lon,
        radius_m: input.radius_m ?? 150, fee_minor: input.fee_minor,
    };
    for (const k of ["where", "area", "instructions", "deliverable", "skills", "attempt_minor"]) {
        const v = input[k];
...
export function jobStatus(job, token, opts = {}) {
...
export function jobReceipt(job, token, opts = {}) {
...
export const runJobSchema = z.object({
    ...
    sandbox: z.boolean().default(true).describe("Default true: costs nothing, needs no account, the job walks the real state machine " +
...
export function createLamdisTools(opts = {}) {
    return {
        lamdis_check_feasible: tool({
            description: "Ask the Lamdis Exchange whether anybody near (lat, lon) could do a piece of physical " +
        ...
        lamdis_run_job: tool({
            description: "Post a job on the Lamdis Exchange: pay a person to check (kind=observe) or to do (kind=do) " +
        ...
        lamdis_job_status: tool({
            description: "Where a Lamdis job stands. Returns taken, submissions and results (each with verified and " +
