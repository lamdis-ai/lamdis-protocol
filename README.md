...
Everything an integrator touches is Apache-2.0: the spec, the SDKs, the
examples, and the receipt format, because a protocol nobody may implement is
not a protocol, and a receipt is only checkable without trusting us if the
rules for checking it are public.

The server is not, and that is deliberate. This is a marketplace, and a
marketplace is worth what its liquidity is worth: one exchange with a hundred
operators serves jobs that fifty exchanges with two each cannot. Forking the
server would fragment the only thing that makes it useful. Each release
converts to MIT after two years, and a private deployment that federates with
the public network is the right answer to self-hosting — not a fork.
