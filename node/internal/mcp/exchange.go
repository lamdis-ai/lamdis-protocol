...
func RegisterExchange(s *sdk.Server, x *Exchange) {
	...
	sdk.AddTool(s, &sdk.Tool{Name: "observe_world",
		Description: "Find out whether something is actually true in the physical " +
			...
			"An agent holding a USDC wallet on Base may pass x402 true to pay " +
			"inline instead: the reply is the x402 payment requirements, and " +
			"retrying POST /v1/tasks with a signed X-PAYMENT header lists the job " +
			"in the same round trip, with no card and no human.\n\n" +
	...
	sdk.AddTool(s, &sdk.Tool{Name: "do_in_world",
		...
		func(ctx context.Context, req *sdk.CallToolRequest, a doArgs) (*sdk.CallToolResult, any, error) {
			out, err := x.call(ctx, "POST", "/v1/tasks", map[string]any{
				...
				"plan_by": a.PlanBy, "sandbox": a.Sandbox, "x402": a.X402,
			})
			return jobResult(out, err)
		})
...
func jobResult(out map[string]any, err error) (*sdk.CallToolResult, any, error) {
	// A 402 is not a failure: it is the x402 price, and the agent that asked
	// for it needs the whole body to sign against, not a one-line error.
	if _, priced := out["x402Version"]; err != nil && priced {
		err = nil
	}
	if err != nil {
