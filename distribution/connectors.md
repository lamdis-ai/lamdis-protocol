...
- Developer mode (verified 2026-09-11): ChatGPT → profile → Settings → Apps &
  Connectors → Advanced settings → toggle Developer Mode → Create; give a name, a
  description, and the server URL `https://exchange.lamdis.ai/mcp` (the `/mcp` path
  is required), no auth. Then review the tool list and enable the ones wanted.
  Available on Plus, Pro, Business, Enterprise and Edu — not the free tier.
